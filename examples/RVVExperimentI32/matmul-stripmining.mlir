...
func.func @test() -> i32 {
  ...
  %aRow = memref.dim %mem_i32, %c0 : memref<20x20xi32>
  %aCol = memref.dim %mem_i32, %c1 : memref<20x20xi32>
  %bRow = memref.dim %mem_i32, %c0 : memref<20x20xi32>
  %bCol = memref.dim %mem_i32, %c1 : memref<20x20xi32>
  ...
  affine.for %idx0 = 0 to %bRow {
    affine.for %idx1 = 0 to %aRow {
      %aEle = affine.load %mem_i32[%idx1, %idx0] : memref<20x20xi32>
      ...
      ^bb0(%avl : i32, %idx : index):
        ...
        %input_vector = vector_exp.predication %mask, %vl : vector<[4]xi1>, i32 {
          %ele = vector.load %mem_i32[%idx0, %idx] : memref<20x20xi32>, vector<[4]xi32>
        ...
        %c_vector = vector_exp.predication %mask, %vl : vector<[4]xi1>, i32 {
          %ele = vector.load %result_mem[%idx1, %idx] : memref<20x20xi32>, vector<[4]xi32>
          vector.yield %ele : vector<[4]xi32>
        } : vector<[4]xi32>
        ...
        vector_exp.predication %mask, %vl : vector<[4]xi1>, i32 {
          vector.store %result_vector, %result_mem[%idx1, %idx] : memref<20x20xi32>, vector<[4]xi32>
          vector.yield
        } : () -> ()
        // Update idx and avl.
